...
class Cannonball1(pygame.sprite.Sprite):
    ...
    def update(self):
        self.now = pygame.time.get_ticks()
        self.speedx += wind.wind[0]
        self.rect.x += self.speedx
        self.speedy += self.accely
        self.speedy -= wind.wind[1]
        self.rect.y += self.speedy
...
class Wind(pygame.sprite.Sprite):
    def __init__(self,):
        ...
        self.wind = np.array([x, y])
        self.mag = np.sqrt(x**2 + y**2)
        if x > 0:
            self.angle = np.arctan(y/x)
        else :
            self.angle = np.arctan(y/x) + np.pi

        if self.mag < 0.1/6:
            self.image = wind_img[2]
        elif self.mag < 0.2/6:
            self.image = wind_img[1]
        else :
            self.image = wind_img[0]
        self.rect = self.image.get_rect()
    ...
    def update(self,):
        ...
        if self.mag < 0.1/6:
            ...
            self.image = wind_img[0]
        new_image = pygame.transform.rotate(self.image, np.rad2deg(self.angle))
        self.image = new_image
...
def show_go_screen():
    ...
    draw_text(screen, "CANNONSHOOTER", 128, WIDTH / 2, HEIGHT / 4)
...
wind_img = []
for i in range(3):
    wind_img_orig = pygame.image.load(path.join(img_dir, f"wind_arrow{i+1}.png")).convert()
    wind_img_orig.set_colorkey(BLACK)
    wind_img.append(pygame.transform.scale(wind_img_orig, (50, 50)))
...
cannon_sound = pygame.mixer.Sound(path.join(snd_dir, "switch_cannon.wav"))
pygame.mixer.music.load(path.join(snd_dir, "Aylex-Glorious(freetouse.com).mp3"))
...
while running:
    ...
    screen.blit(wind.image, wind.rect)
